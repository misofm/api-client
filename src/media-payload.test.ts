...
test("covers preserve blob references without requiring a host", () => {
  const cover = { still: { kind: "blob", blobId }, animated: { kind: "blob", blobId } };
  expect(coverSchema.parse(cover)).toEqual(cover);
  expect(coverSchema.safeParse({ ...cover, still: { kind: "blob", blobId: "bad" } }).success).toBe(false);
});
...
test("full master metadata retains encrypted and unencrypted data", () => {
  for (const confidentiality of [{ $kind: "Unencrypted", Unencrypted: true }, { $kind: "Encrypted", Encrypted: { sealed_dek: [1, 2, 3] } }]) {
